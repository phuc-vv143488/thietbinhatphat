...
export async function generateStaticParams() {
  return productDatas.flatMap(menu =>
...
const { submenu, slug } = await params; // Phải await ở đây
  const menu = productDatas.find(m => m.submenu === submenu);
...
export default async function ProductDetailPage({params}: PageProps) {
    ...
    const product = (productDatas.find((p) => p.submenu === submenu) || {})?.products.find(p => p.slug === slug);
